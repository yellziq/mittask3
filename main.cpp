...
void result(map<int, list<int>> l_Adj) // Функция для вывода степени каждой вершины
{
    for (auto it = l_Adj.begin(); it != l_Adj.end(); it++) // Перебор всех вершин в списке смежности
    {
        int count = 0; // Счетчик степени вершины
        for (auto it_1 = it->second.begin(); it_1 != it->second.end(); it_1++) // Перебор всех смежных вершин текущей вершины
            count++; // Увеличиваем счетчик на 1 за каждое ребро
        cout << it->first << ": " << count << endl; // Выводим номер вершины и ее степень
    }
}
...
int main()
{
    ...
    result(l_Adj); // Выводим степень каждой вершины
